...
public class AttireGenerator {
    ...
    public void calculateSuggestionsForAllUsers() {
        UserRepository.getInstance().getUsers().forEach(this::generateSuggestions);
    }

    public void generateSuggestions(User user) {
        Set<Attire> attires = user.getWardrobes().stream().map(this::getSuggestions).flatMap(Collection::stream).collect(Collectors.toSet());
        user.setDailySuggestions(attires);
    }
}
